...
class Ext2Tester(unittest.TestCase):
    ...
    def test_2_super_bloc_read_super(self):
        ...
        self.assertEqual(sb.s_blocks_per_group, 8192)
        self.assertEqual(sb.s_inodes_count, 2048)
        self.assertEqual(sb.s_blocks_count, 8192)
        self.assertEqual(sb.s_r_blocks_count, 409)
        self.assertEqual(sb.s_free_blocks_count, 259)
        self.assertEqual(sb.s_free_inodes_count, 1440)
        self.assertEqual(sb.s_first_data_block, 1)
        self.assertEqual(sb.s_log_block_size, 0)
        self.assertEqual(sb.s_inodes_per_group, 2048)
        self.assertEqual(sb.s_magic, 0xEF53)
        self.assertEqual(sb.s_state, 1)
        ...
        self.assertEqual(sb.s_inode_size, 128)
    ...
    def test_7_fs_lookup_entry(self):
        ...
        inode = ext2fs.lookup_entry(ext2fs.inodes_list[426], "ps.c")
        self.assertEqual(inode, LOOKUPINODE1)
        # lookup_entry, inode 113 ("/usr/src/linux/fs/minix"), lookup for namei.c
        inode = ext2fs.lookup_entry(ext2fs.inodes_list[113], "namei.c")
